# 事業仮説を組み立てる (Create Thesis)

## 目的・概要

投資家が、蓄積された複数の `Thought`（意味づけ）を束ね、1つの `Company` についての事業仮説（`Thesis`）へ収斂させる。裸の Finding や単一の Thought はスタンスを持たないため、Thesis は複数の Thought を根拠として積み上げて初めて成立する。

Thesis の核心は、何が起きたら仮説が崩れるか（`invalidation`）、何が起きたら確度が上がるか（`confirmation`）を対で必ず明文化することにある。反証条件のみだと判定基準が片側に偏り、statusを前進させる方向にしか働かなくなるため。
...
5. システムは、`statement`/`invalidation`/`confirmation`/`body` がいずれも空文字列でないことを検証する。
...
- **3a. 必須項目のいずれかが空の場合:**
  システムは保存処理を行わず、未入力の項目を明示してエラーを返す。
