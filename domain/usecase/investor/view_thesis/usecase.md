...
4. システムは、この Thesis を `validatesThesisId` に持つ `Signal` の一覧（Phase 3以降）、および `thesisId` に持つ `Prediction` の一覧（Phase 5以降）が存在すれば、あわせて取得する。
...
- **4a. 紐づく Signal / Prediction が存在しない場合（未導入Phase、または未作成）:**
