...
投資家が、決算資料や適時開示などを読んで気づいた数値（売上高、稼働率、求人数等）を、`Thesis` の前提を検証するための時系列データポイント（`Signal`）として記録する。Signal は単体では意味を持たず、「この指標は Thesis のどの前提を検証するためのものか」を明示して初めて記録できる。
...
2. 投資家は、指標の分類（`category`: `financial` / `operational` / `leading`）、指標名（`metric`）、対象期間・時点（`period`）、値（`value`）、単位（`unit`）を入力する。
