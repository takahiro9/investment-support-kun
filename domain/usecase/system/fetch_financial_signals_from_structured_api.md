# 構造化データAPIから Signal を自動取得する (Fetch Financial Signals from Structured API)

## 目的・概要

システムが定期的に構造化データAPI（EDINET等）を呼び出し、`financial` カテゴリ（売上高・営業利益率等の決算数値）の指標を取得し、`Signal` として蓄積する。このカテゴリは構造化APIから安価かつ正確に取得できるため、自然言語処理（LLM抽出）を経由しない専用の取得経路を持つ（対照的に `leading`/`operational` カテゴリの取得は [extract_signal_from_finding](extract_signal_from_finding.md) が担う）。
...
1. システムは、一定間隔（例: 決算発表スケジュールに合わせて）で取得処理をトリガーする。
2. システムは、`financial` カテゴリの Signal 取得が設定されている `Company` の一覧を取得する。
3. システムは、各 `Company` に対して以下の処理を繰り返す:
   1. 構造化データAPIを呼び出し、対象 `Company` の決算数値等の最新値を取得する。
