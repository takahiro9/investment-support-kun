...
Thoughtは**1つ以上のFindingに根ざす**（`findingIds`）。複数の事実を並べて1つの意見・観察を述べることができる。関連付け先はCompany・Business・Sector・Themeの4種類があり、それぞれ0個以上（`companyIds`/`businessIds`/`sectorIds`/`themeIds`）で持つ。考察そのものは**本文（`body`）**に文章として綴る。

Finding↔Company/Business/Sector/Themeの関連はFinding側ではなくThought側に存在する。したがって、あるCompany/Business/Sector/Themeに属するFinding群は**Thoughtを辿って**求める。1つのThoughtを複数のCompany/Business/Sector/Themeに結びつけることもできる。
...
[Business](business.md)のドライバーツリーの各ノードをFindingで埋めていく作業は、Thoughtが`driverNodeIds`を持つことで表現する。あるThoughtが「このFindingは事業Aの数量に関する情報だ」と意味づけた場合、`businessIds`に対象Businessを、`driverNodeIds`にそのBusinessのツリー内のノードidを入れる。Business・Finding自身はノードへの参照を持たない（関連付けはすべてThoughtが担うという原則を、ドライバーツリーのノードにも適用する）。
...
| `businessIds` | string[] (FK) | ❌ | 紐づくBusiness群（0個以上） |
| `sectorIds` | string[] (FK) | ❌ | 紐づくSector群（0個以上） |
| `themeIds` | string[] (FK) | ❌ | 紐づくTheme群（0個以上、Phase 4以降） |
| `driverNodeIds` | string[] | ❌ | 紐づくBusinessのドライバーツリーのノードid群（`businessIds`が指すBusinessのツリー内のノードを指す） |
...
- `companyIds`/`businessIds`/`sectorIds`/`themeIds` はすべて空でもよい（関連付けを持たないThoughtも許容する）が、実務上は少なくとも1つに紐づけることが推奨される
- `driverNodeIds` を指定する場合、`businessIds` に対応するBusinessが含まれていること（ドライバーツリーのノードは特定Businessのツリーに属するため）
...
- **Company / [Business](business.md) / Sector / Theme** — `companyIds`/`businessIds`/`sectorIds`/`themeIds`を介して関連付ける。Company/Business/Sector/Theme↔Findingの関連はこのThoughtを介してのみ成立する
