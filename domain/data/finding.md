...
「気になった情報アイテム」の最小単位であり、**文脈を持たない事実のスナップショット**。登録された情報源（[Source](source.md)）から自動収集されたものに限らず、手動で登録した記事・メモ・適時開示・市場データなども含む、気になった対象全般を受け止める器である。Web記事・メモ・PDF・適時開示・市場データなど多様な形式の情報を統一的に扱う。
...
| `market_data` | 株価・バリュエーション等の数値スナップショット |
