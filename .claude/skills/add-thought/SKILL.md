...
4. 任意でこの Thought が関係する `companyIds` / `sectorIds` を確認する（id が分からなければ `list-companies`/`list-sectors` skill で引く）。`themeIds` は Theme エンティティが未実装（Phase 4）のため、現時点では通常指定しない。
...
7. 以下を実行する:
   ...
   uv run python scripts/thoughts.py add --id <uuid生成結果> \
     ...
     [--company-ids "<id1>,..."] [--sector-ids "<id1>,..."] \
...
8. コマンドが非ゼロで終了した場合、stderr の `{"errors": [...]}` を伝え、再入力を促す。よくある失敗:
   ...
   - 指定した `companyIds`/`sectorIds` が存在しない
