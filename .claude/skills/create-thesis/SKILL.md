---
name: create-thesis
description: Bundle accumulated Thoughts into a single business Thesis for a Company — invalidation/confirmation conditions. Use when the investor is ready to commit to a testable business hypothesis, not just scattered observations.
---
...
7. 以下を実行する:
   ...
   uv run python scripts/theses.py register --id <uuid生成結果> --company-id "<companyId>" \
     --statement "<statement>" --invalidation "<invalidation>" --confirmation "<confirmation>" \
