---
name: record-signal
description: Manually record a time-series Signal (financial/operational/leading/market metric) that validates a specific Thesis assumption. Use when the investor read a number in a disclosure/IR doc/market data and wants to track it against a hypothesis.
---
...
3. 指標の分類（`category`: `financial`/`operational`/`leading`/`market`）、指標名（`metric`）、対象期間・時点（`period`、例: `2027Q2`）、値（`value`）、任意で単位（`unit`）を確認する。
